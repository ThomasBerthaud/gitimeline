...
new Vue({
  ...
  created: function() {
    this.$http
      .get(apiUrl + "/GitHubColors/ext")
      .then(request => {
        gitHubColors = request.body;
      })
      .catch(console.error);
  },
  computed: {
  ...
  methods: {
    displayGraph: function() {
      ...
      let self = this;
      let filesTreeUrl = `${apiUrl}/repos/${this.repoInfos.owner}/${
        this.repoInfos.repo
      }/commits/${this.commitSelected}/files`;
    ...
    displayFile: function(file) {
      let url = `${apiUrl}/repos/${this.repoInfos.owner}/${
        this.repoInfos.repo
      }/commits/${this.commitSelected}/files/${file.path}`;
      this.$http
        .get(url)
        .then(request => {
        ...
        .catch(() => {
          let errorMessage =
            "Ce fichier n'a pas encore été crée ou a été supprimé !";
          this.selectedFile = {
            name: file.name,
            path: file.path,
            size: 0,
